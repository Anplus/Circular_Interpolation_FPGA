...
handles
%% transaction
fwrite(handles.o_SerialPort,handles.shape,'uint8');%shape
pause(0.1);
fwrite(handles.o_SerialPort,handles.method,'uint8');%method
pause(0.1);
fwrite(handles.o_SerialPort,bitand(handles.Xs, 255),'uint8');%Xs low
pause(0.1);
%%
if(handles.Xs>=0)
    c = num2str(dec2bin(handles.Xs));
    for i = 1 : 1 : 16-length(c)
        c = strcat('0',c);
    end
    c = c(1:8);
else
    c = num2str(dec2bin(65535+handles.Xs));
    c = c(1:8);
end
fwrite(handles.o_SerialPort,uint8(str2num(c)),'uint8');%Xs high
pause(0.1);
%%
fwrite(handles.o_SerialPort,bitand(handles.Ys, 255),'uint8');%Ys low
pause(0.1);
%%
if(handles.Ys>=0)
    c = num2str(dec2bin(handles.Ys));
    for i = 1 : 1 : 16-length(c)
        c = strcat('0',c);
    end
    c = c(1:8);
else
    c = num2str(dec2bin(65535+handles.Ys));
    c = c(1:8);
end
fwrite(handles.o_SerialPort,uint8(str2num(c)),'uint8');%Ys high
pause(0.1);
%%
fwrite(handles.o_SerialPort,bitand(handles.Xe, 255),'uint8');%Xe low
pause(0.1);
%%
if(handles.Xe>=0)
    c = num2str(dec2bin(handles.Xe));
    for i = 1 : 1 : 16-length(c)
        c = strcat('0',c);
    end
    c = c(1:8);
else
    c = num2str(dec2bin(65535+handles.Xe));
    c = c(1:8);
end
fwrite(handles.o_SerialPort,uint8(str2num(c)),'uint8');%Xe high
pause(0.1);
%%
fwrite(handles.o_SerialPort,bitand(handles.Ye, 255),'uint8');%Ye low
pause(0.1);
%%
if(handles.Ye>=0)
    c = num2str(dec2bin(handles.Ye));
    for i = 1 : 1 : 16-length(c)
        c = strcat('0',c);
    end
    c = c(1:8);
else
    c = num2str(dec2bin(65535+handles.Ye));
    c = c(1:8);
end
fwrite(handles.o_SerialPort,uint8(str2num(c)),'uint8');%Ye high
pause(0.1);
%%
fwrite(handles.o_SerialPort,handles.direct,'uint8');%direct
pause(0.1);
fwrite(handles.o_SerialPort,handles.speed,'uint8');%max_speed
pause(0.1);
fwrite(handles.o_SerialPort,handles.accelerate,'uint8');%accelerate
pause(0.1);
...
%% out
handles.move = fread(handles.o_SerialPort,1);
...
elseif handles.move ==6
    handles.starty = handles.starty - 1 ;
elseif handles.move ==0
    msgbox('插补完成！','提示','warn');
end
handles
